...
select b.*,
exists (
select 1 from {{ ref('stg_streams') }} b2
...
select * from marked where has_nearby_dup = false


{% if is_incremental() %}
and started_at > (select coalesce(max(started_at), to_timestamp_ntz('1900-01-01')) from {{ this }})
{% endif %}